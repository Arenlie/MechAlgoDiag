...
def detect_bearing_fault(acc_signal, fs, notice_th=None, warn_th=None):
    ...
    if (notice_th <= acc_kurtosis <= warn_th) and acc_p_factor >= 4:
        alarm_level = "注意"
        alarm_threshold = notice_th
    elif acc_kurtosis >= warn_th and acc_p_factor >= 4:
        alarm_level = "警告"
        alarm_threshold = warn_th
    else:
        alarm_level = None
        alarm_threshold = None
    alarm_message = f"轴承磨损峭度能量【{alarm_level}级】报警，峭度能量为{round(acc_kurtosis, 2)}m/s²，阈值上限为{round(alarm_threshold, 2)}m/s²，超标量{round(round(acc_kurtosis, 2) - round(alarm_threshold, 2), 2)}m/s²" if alarm_level else None

    results = {
        "type": "轴承磨损峭度能量",
        "energy": round(acc_kurtosis, 4),
